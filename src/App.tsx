...
import { User, Worksite, TimeRecord, useGeolocation, calculateDistance } from './types';
import { 
...
const generateRecordPDF = (record: TimeRecord, user: User) => {
  ...
  doc.setFontSize(12); doc.text(`Tipo: ${record.type === 'IN' ? 'ENTRADA' : 'SALIDA'}`, 20, 100); doc.text(`Fecha: ${new Date(record.timestamp).toLocaleDateString('es-ES')}`, 20, 107); doc.text(`Hora: ${new Date(record.timestamp).toLocaleTimeString('es-ES')}`, 20, 114); doc.text(`Sede: ${record.worksite_name}`, 20, 121); doc.text(`Distancia: ${(record.distance || 0).toFixed(1)}m`, 20, 128); doc.text(`Coordenadas: ${(record.latitude || 0).toFixed(6)}, ${(record.longitude || 0).toFixed(6)}`, 20, 135); doc.text(`Método: ${record.is_manual ? 'Manual' : 'Automático (GPS)'}`, 20, 142);
  if (record.notes) { doc.text('Notas:', 20, 155); doc.setFontSize(10); doc.setTextColor(100); doc.text(doc.splitTextToSize(record.notes, 170), 20, 162); }
...
const generateFullReportPDF = (records: TimeRecord[], user: User, periodLabel?: string) => {
  ...
  if (!isConsolidated) { doc.text(`Empleado: ${user.name} (${user.employee_id})`, 20, 30); } else { doc.text(`Informe Consolidado de Administración`, 20, 30); }
  ...
  Object.values(recordsByUser).forEach((userData, userIdx) => {
    ...
    const tableData = sortedRecords.map((r, idx) => {
      ...
      let dayTotalStr = '';
      if (dayLastRecordIndex[dateStr] === idx && dayTotalMs > 0) dayTotalStr = `${Math.floor(dayTotalMs / 3600000)}h ${Math.floor((dayTotalMs % 3600000) / 60000)}m`;
      else if (dayLastRecordIndex[dateStr] === idx && dayTotalMs === 0) dayTotalStr = '0h 0m';
      
      let typeText = r.type === 'IN' ? 'Entrada' : 'Salida';
      if (r.estado_extra === 'PENDIENTE') typeText += ' (+Extras)';
...
const Login = ({ onLogin }: { onLogin: (user: User) => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { const user = await res.json(); if (mode === 'ADMIN' && user.role !== 'ADMIN') return setError('Se requieren credenciales de administrador'); onLogin(user); } 
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-['Quicksand']">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 font-medium">{mode === 'ADMIN' ? 'Acceso restringido para gestión de plataforma' : 'Sistema de fichaje con validación de geolocalización'}</p>
        </div>
        <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800 shadow-2xl backdrop-blur-xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2"><label className="text-sm font-bold text-slate-400 ml-1">Email</label><div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" /><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-slate-800/50 border border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-[#ff8c00] transition-all" /></div></div>
            <div className="space-y-2"><label className="text-sm font-bold text-slate-400 ml-1">Contraseña</label><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" /><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-800/50 border border-slate-700 rounded-2xl py-4 pl-12 pr-12 text-white outline-none focus:border-[#ff8c00] transition-all" /></div></div>
            <button type="submit" className="w-full bg-gradient-to-r from-[#ff8c00] to-orange-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2"><span>Entrar</span><ChevronRight className="w-5 h-5" /></button>
          </form>
        </div>
        <div className="text-center">
          <button onClick={() => { setMode(mode === 'USER' ? 'ADMIN' : 'USER'); setEmail(''); setPassword(''); }} className="text-sm font-bold text-slate-400 hover:text-white transition-colors">
            {mode === 'USER' ? '¿Eres administrador? Acceder al Panel' : 'Volver al Inicio de Sesión'}
          </button>
        </div>
      </motion.div>
    </div>
    ...
    <div className="flex-1 flex flex-col p-4 space-y-6 max-w-md mx-auto w-full font-['Quicksand'] pb-24">
      ...
      <section className="space-y-4 px-2">
        <div className="relative">
          <select value={selectedWorksite} onChange={(e) => setSelectedWorksite(Number(e.target.value))} className="w-full bg-slate-900 border border-orange-500/20 text-white rounded-xl px-4 py-3 outline-none appearance-none font-medium">
        ...
        <div className={`flex items-center gap-2 justify-center py-2 px-4 rounded-lg border ${canClockIn ? 'bg-green-500/10 border-green-500/20' : 'bg-orange-500/5 border-orange-500/10'}`}>
          ...
          <p className={`text-xs font-medium ${canClockIn ? 'text-green-500' : 'text-slate-400'}`}>{canClockIn ? `¡Estás a ${distance?.toFixed(1)}m! Puedes fichar.` : `Acércate a menos de ${currentSite?.radius || 10}m (Estás a ${distance !== null ? distance.toFixed(1) : '?'}m)`}</p>
        </div>
        {geoError && <p className="text-red-500 text-[10px] font-bold mt-2 uppercase text-center">⚠️ Error GPS: {geoError}</p>}
      </section>
      ...
      <section className="grid grid-cols-2 gap-4">
        <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
          <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">Horas Hoy</p>
          <p className="text-2xl font-black text-white">{stats.todayStr || '0h 0m'}</p>
        </div>
        <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
          <p className="text-[10px] uppercase font-bold text-slate-500 mb-1">Esta Semana</p>
          <p className="text-2xl font-black text-[#ff8c00]">{stats.weekStr || '0h 0m'}</p>
        </div>
      </section>
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {records.slice(0, 3).map(record => (
            <div key={record.id} className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-slate-800">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center">{record.type === 'IN' ? <LogIn className="text-[#ff8c00]" /> : <LogOut className="text-[#ff8c00]" />}</div>
                <div><p className="font-semibold text-sm">{new Date(record.timestamp).toLocaleDateString('es-ES')}</p><p className="text-xs text-slate-500">{new Date(record.timestamp).toLocaleTimeString('es-ES', {hour: '2-digit', minute: '2-digit'})} - {record.worksite_name}</p></div>
              </div>
...
const ActiveSession = ({ user, onFinish, onDiscard, startTime }: { user: User, onFinish: (notes: string) => void, onDiscard: () => void, startTime: Date }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => { const diff = new Date().getTime() - startTime.getTime(); setElapsed({ h: Math.floor(diff / 3600000), m: Math.floor((diff % 3600000) / 60000), s: Math.floor((diff % 60000) / 1000) }); }, 1000);
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-8 font-['Quicksand'] pb-24">
      ...
      <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full h-32 p-4 bg-slate-800 border border-slate-700 rounded-xl outline-none" placeholder="Añade notas del turno..." />
      <button onClick={() => onFinish(notes)} className="mt-8 w-full bg-[#ff8c00] py-4 rounded-xl font-bold flex items-center justify-center gap-2"><TimerOff /> Finalizar Turno</button>
    </div>
  );
};
...
const HistoryView = ({ records, user, onSelectRecord }: { records: TimeRecord[], user: User, onSelectRecord: (record: TimeRecord) => void }) => (
  <div className="flex-1 px-4 py-6 space-y-4 pb-24 font-['Quicksand']">
    ...
    <div className="space-y-4">
      {records.map(record => (
        <div key={record.id} onClick={() => onSelectRecord(record)} className={`bg-slate-800 rounded-xl border-l-4 p-4 flex justify-between cursor-pointer border border-slate-700 ${record.type === 'IN' ? 'border-l-green-500' : 'border-l-red-500'}`}>
          <div className="flex items-center gap-4">
            <div className="bg-slate-900 w-14 h-14 rounded-lg flex flex-col items-center justify-center"><span className="text-xl font-black">{new Date(record.timestamp).getDate()}</span><span className="text-[10px] text-slate-500 uppercase">{new Date(record.timestamp).toLocaleDateString('es-ES', {month:'short'})}</span></div>
    ...
    <div className="p-6 space-y-6">
      ...
      {record.minutos_extra ? (
        <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-xl">
          <p className="text-orange-500 text-[10px] uppercase font-bold">Horas Extra</p>
          <p className="font-bold text-orange-400">+{record.minutos_extra} min ({record.estado_extra})</p>
        </div>
      ) : null}
...
const WeeklySummaryView = ({ records, user, showToast }: { records: TimeRecord[], user: User, showToast: (msg: string, type: 'success' | 'error') => void }) => (
  <div className="flex-1 p-6 text-center space-y-6 font-['Quicksand'] pb-24">
    ...
    <button onClick={() => { generateFullReportPDF(records, user); showToast('Informe PDF Generado', 'success'); }} className="w-full bg-[#ff8c00] py-4 rounded-xl font-bold flex justify-center gap-2"><FileText /> Descargar Informe Mensual</button>
  </div>
);
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col font-['Quicksand'] relative">
                 ...
                 <>
                   {activeTab === 'home' && (
                     <div className="p-6 space-y-6 pb-24">
                       ...
                       <div className="grid grid-cols-2 gap-4">
                         <button onClick={()=>setActiveTab('admin-users')} className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-2"><Users className="text-[#ff8c00] w-8 h-8"/><span className="font-bold text-sm">Usuarios</span></button>
                         <button onClick={()=>setActiveTab('admin-worksites')} className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-2"><Building2 className="text-[#ff8c00] w-8 h-8"/><span className="font-bold text-sm">Sedes</span></button>
                         <button onClick={()=>setActiveTab('admin-records')} className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-2"><FileText className="text-[#ff8c00] w-8 h-8"/><span className="font-bold text-sm">Logs Diarios</span></button>
                         <button onClick={()=>setActiveTab('admin-export')} className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col items-center gap-2"><Download className="text-[#ff8c00] w-8 h-8"/><span className="font-bold text-sm">Exportar PDF</span></button>
                       </div>
                       <button onClick={()=>setActiveTab('admin-requests')} className={`w-full p-6 rounded-3xl font-bold flex justify-between items-center ${pendingReqs.length > 0 ? 'bg-orange-500/20 text-[#ff8c00] border border-orange-500/50' : 'bg-slate-900 border border-slate-800 text-slate-500'}`}><span>Revisar Alertas Extra</span><span className="bg-slate-950 px-3 py-1 rounded-full">{pendingReqs.length}</span></button>
                     </div>
                   )}
                   ...
                   {activeTab === 'admin-worksites' && <WorksiteManagementView worksites={adminWorksites} onBack={()=>setActiveTab('home')} onAdd={async(d:any)=>{await fetch('/api/admin/worksites',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});fetchAdminData();}} onUpdate={async(id:any,d:any)=>{await fetch(`/api/admin/worksites/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});fetchAdminData();}} onDelete={async(id:any)=>{if(confirm('¿Borrar?')){await fetch(`/api/admin/worksites/${id}`,{method:'DELETE'});fetchAdminData();}}} />}
                   {activeTab === 'admin-records' && <AdminRecordsListView records={allRecords} users={adminUsers} onSelectRecord={setSelectedRecord} onBack={()=>setActiveTab('home')} />}
                   {activeTab === 'admin-export' && <ExportView records={allRecords} showToast={(m:string,t:any)=>setToast({message:m,type:t})} onBack={()=>setActiveTab('home')} />}
                   {activeTab === 'admin-requests' && <PendingRequestsView requests={pendingReqs} onSelectRecord={setSelectedRecord} onBack={()=>setActiveTab('home')} onActionComplete={async(id:any,status:any)=>{await fetch('/api/admin/records/approve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,status})});fetchAdminData();}} />}
      ...
      {!selectedRecord && (
        <nav className="fixed bottom-0 w-full bg-slate-950/95 border-t border-slate-800 flex justify-around p-4 z-50 backdrop-blur-md">
          <button onClick={()=>setActiveTab('home')} className={`flex flex-col items-center gap-1 ${activeTab === 'home' ? 'text-[#ff8c00]' : 'text-slate-500'}`}><Home className="w-6 h-6"/><span className="text-[10px] font-bold uppercase">Inicio</span></button>
          <button onClick={()=>setActiveTab(user.role === 'ADMIN' ? 'admin-records' : 'history')} className={`flex flex-col items-center gap-1 ${activeTab === 'history' || activeTab === 'admin-records' ? 'text-[#ff8c00]' : 'text-slate-500'}`}><FileText className="w-6 h-6"/><span className="text-[10px] font-bold uppercase">Registros</span></button>
          {user.role === 'USER' && <button onClick={()=>setActiveTab('summary')} className={`flex flex-col items-center gap-1 ${activeTab === 'summary' ? 'text-[#ff8c00]' : 'text-slate-500'}`}><BarChart3 className="w-6 h-6"/><span className="text-[10px] font-bold uppercase">Resumen</span></button>}
          <button onClick={()=>setActiveTab('profile')} className={`flex flex-col items-center gap-1 ${activeTab === 'profile' ? 'text-[#ff8c00]' : 'text-slate-500'}`}><UserIcon className="w-6 h-6"/><span className="text-[10px] font-bold uppercase">Perfil</span></button>
        </nav>
      )}
